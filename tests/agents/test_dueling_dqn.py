...
from torchforce.agents import DuelingDQN
...
def test_dueling_dqn_agent_instantiation():
    network = Network()
    memory = ExperienceReplay(max_size=5)

    DuelingDQN(Discrete(4), Discrete(3), neural_network=network, memory=memory)
    DuelingDQN(Discrete(4), Discrete(3))


def test_dueling_dqn_agent_instantiation_error_action_space():
    network = Network()
    memory = ExperienceReplay(max_size=5)

    with pytest.raises(TypeError):
        DuelingDQN("ACTION_SPACE_ERROR", Discrete(3), neural_network=network, memory=memory)


def test_dueling_dqn_agent_instantiation_error_observation_space():
    network = Network()
    memory = ExperienceReplay(max_size=5)

    with pytest.raises(TypeError):
        DuelingDQN(Discrete(1), "OBSERVATION_SPACE_ERROR", neural_network=network, memory=memory)


def test_dueling_dqn_agent_instantiation_error_neural_network():
    memory = ExperienceReplay(max_size=5)

    with pytest.raises(TypeError):
        DuelingDQN(Discrete(4), Discrete(3), neural_network="NEURAL_NETWORK_ERROR", memory=memory)


def test_dueling_dqn_agent_instantiation_error_memory():
    network = Network()

    with pytest.raises(TypeError):
        DuelingDQN(Discrete(4), Discrete(3), neural_network=network, memory="MEMORY_ERROR")


def test_dueling_dqn_agent_instantiation_error_loss():
    network = Network()
    memory = ExperienceReplay(max_size=5)

    with pytest.raises(TypeError):
        DuelingDQN(Discrete(4), Discrete(3), neural_network=network, memory=memory, loss="LOSS_ERROR")


def test_dueling_dqn_agent_instantiation_error_optimizer():
    network = Network()
    memory = ExperienceReplay(max_size=5)

    with pytest.raises(TypeError):
        DuelingDQN(Discrete(4), Discrete(3), neural_network=network, memory=memory, optimizer="OPTIMIZER_ERROR")


def test_dueling_dqn_agent_instantiation_error_greedy_exploration():
    network = Network()
    memory = ExperienceReplay(max_size=5)
...
def test_dueling_dqn_agent_instantiation_custom_loss():
    ...
    DuelingDQN(Discrete(4), Discrete(3), neural_network=network, memory=memory, loss=nn.MSELoss())


def test_dueling_dqn_agent_instantiation_custom_optimizer():
    network = Network()
    memory = ExperienceReplay(max_size=5)
...
def test_dueling_dqn_agent_getaction():
    ...
    agent = DuelingDQN(Discrete(4), Discrete(3), neural_network=network, memory=memory, greedy_exploration=Greedy())
...
def test_dueling_dqn_agent_getaction_non_greedy():
    network = Network()
    memory = ExperienceReplay(max_size=5)
...
def test_dueling_dqn_agent_learn():
    network = Network()
    memory = ExperienceReplay(max_size=5)
...
def test_dueling_dqn_agent_episode_finished():
    network = Network()
    memory = ExperienceReplay(max_size=5)
...
def test_agent_save_load():
    ...
    with pytest.raises(FileNotFoundError):
        DuelingDQN.load(file_name="deed.pt")
    with pytest.raises(FileNotFoundError):
        DuelingDQN.load(file_name="deed.pt", dire_name="/Dede/")


def test__str__():
    agent = DuelingDQN(Discrete(4), Box(1, 10, (4,)))
